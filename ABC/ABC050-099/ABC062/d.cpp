...
int main(){
    ...
    // 境目に, SMSL := 左側上位 N 個の総和と, SMSR := 右側下位 N 個の総和を管理する
    SeparatedMultisetSum<ll> SMSL, SMSR;
    for(int i = 0; i < N; i++) SMSL.insert_high(A[i]);
    for(int i = N; i < 2 * N; i++) SMSR.insert_low(A[i]);
    for(int i = 2 * N; i < 3 * N; i++) SMSR.insert_high(A[i]);
    debug(SMSR.low, SMSR.high);

    ll ans = SMSL.sum_high() - SMSR.sum_low();
    for(int i = N; i < 2 * N; i++){
        // A[i] を SMSL に追加して, SMSR から削除
        SMSL.insert_low(A[i]), SMSR.erase_high(A[i]);
        ans = max(ans, SMSL.sum_high() - SMSR.sum_low());
    }
